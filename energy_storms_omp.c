...
int main(int argc, char *argv[]) {
    ...
    for( i=0; i<num_storms; i++) {
        ...
        for( j=0; j<storms[i].size; j++ ) {
            ...
            #pragma omp parallel for default(none) shared(layer,layer_size, position,energy) private(k)
            for( k=0; k<layer_size; k++ ) {
        ...
        #pragma omp parallel for default(none) shared(layer, layer_size, layer_copy) 
	for( k=0; k<layer_size; k++) { 
