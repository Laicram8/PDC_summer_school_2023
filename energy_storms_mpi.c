...
int main(int argc, char *argv[]) {
    ...
    float *layer_sum = (float *)malloc( sizeof(float) * layer_size );

    if ( layer == NULL || layer_copy == NULL || layer_sum == NULL) {
    ...
    for( i=0; i<num_storms; i++) {
        ...
        /* For each particle, split among ranks */

        /* Check if number of particles can be divided by number of ranks*/
        if (storms[i].size < size) {
           fprintf(stderr,"Error: More ranks than particles in storm.\n");
           exit( EXIT_FAILURE );
        }
        remainder = storms[i].size % size;
        n = storms[i].size/size;
        if (rank == 0) {
            lb = rank*n;
        } else {
            lb = rank*n+remainder;
        }
        ub = (rank+1)*n+ remainder;
        //printf("For loop of rank %d goes from %d to %d \n", rank, lb, ub-1);
        for( j=lb; j<ub; j++ ) {
        ...
        /* 4.3 Reduce results (sum of each cell in array) from all ranks to layer_sum for rank 0 */
        //printf("layer before reduce: %f \n", layer[0]);
        //printf("layer_sum before reduce: %f \n", layer_sum[0]);
        MPI_Reduce(layer, layer_sum, layer_size, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
        //printf("layer_sum after reduce: %f \n", layer_sum[0]);
        /* 4.4. Locate the maximum value in the layer, and its position */
        if (rank == 0) {
           for( k=1; k<layer_size-1; k++ ) {
              /* Check it only if it is a local maximum */
              if ( layer_sum[k] > layer_sum[k-1] && layer_sum[k] > layer_sum[k+1] ) {
                 if ( layer_sum[k] > maximum[i] ) {
                    maximum[i] = layer_sum[k];
                    positions[i] = k;
                 }
              }
           }
    ...
    #ifdef DEBUG
    debug_print( layer_size, layer_sum, positions, maximum, num_storms );
    #endif
